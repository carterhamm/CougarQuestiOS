 ...
 * its last child. The CSS for .glass-rim uses --ang (set here per frame) to
 * draw a conic-gradient highlight on the segment of the rim facing the
 * cursor — additive on top of the baseline outline that's always present.
 *
 * --ang     bearing tile-center → cursor in CSS conic terms
 *           (0=N, 90=E, 180=S, 270=W)
 *
 * Void HTML elements (<input>, <img>, etc.) cannot have children, so they
 * keep only the baseline outline rim — no cursor highlight.
 */
